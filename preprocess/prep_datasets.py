...
def make_sememe_idx(inp_train_path, inp_valid_path, inp_test_path):
    ...
    idx2sememe = {v: k for k, v in sememe2idx.items()}
...
def make_dataset(inp_path, word2idx, sememe2idx, max_def_len, max_sem_len):
    ...
    for word, sememes, definition in data_content:
        ...
        def_len_list.append(len(definition))
    word_sememe_list_padding = padding(word_sememe_list, max_sem_len + 2)
    definition_list_padding = padding(definition_list, max_def_len + 2)
    word_sememe_array = np.array(word_sememe_list_padding)
    definition_array = np.array(definition_list_padding)
    def_len_array = np.array(def_len_list)
    print('Word Sememe Array: ', word_sememe_array.shape)
    print('Definition Array: ', definition_array.shape)
    print('Definition Len Array: ', def_len_array.shape)
    data = [word_sememe_array, definition_array, def_len_array]
    return data
...
def main(args):
    ...
    np.savez(
        save_train_path,
        word_sememes=train_data[0],
        definitions=train_data[1],
        def_len=train_data[2])
    np.savez(
        save_valid_path,
        word_sememes=valid_data[0],
        definitions=valid_data[1],
        def_len=train_data[2])
    np.savez(save_test_path, word_sememes=test_data)
    return 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--unk_thres', type=int, default=-1)
